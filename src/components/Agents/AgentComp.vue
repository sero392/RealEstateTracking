<template>
  <v-layout wrap class="mw-200 ml-2">
    <v-flex class="pa-0" v-for="(item, index) in openAgents" v-bind:key="index">
      <v-tooltip bottom>
        <template v-slot:activator="{ on, attrs }">
          <v-avatar dark size="25" class="ml-1 mouse-pointer" v-bind="attrs" v-on="on" v-bind:color="item.IsSelected ? 'blue-grey lighten-4' : item.fields.color
    ...
    <v-flex class="pa-0">
      <v-avatar dark size="25" color="blue-grey lighten-2" class="ml-1 mouse-pointer" v-on:click="toggleAvatar">
