    ...
    <v-container fluid>
      <v-row dense>
        <v-col>
          ...
          <span class="caption"
            >{{ edit.id ? "Edit" : "Create" }} An Appointment</span
          >
        </v-col>
      </v-row>
      <v-divider></v-divider>
      <v-row
        v-if="!edit.contact || edit.contact.length == 0"
        dense
        class="mt-2"
      >
        <v-col cols="12" class="d-flex flex-column align-start">
          <fieldset class="w-100 pa-1">
            ...
            <select id="contact-select" class="w-100 main-combobox mt-1">
              <option
                v-for="(item, index) in tempContacts"
                v-bind:key="index"
                v-bind:value="index"
              >
                {{
      ...
      <v-row
        v-else
        class="mt-2"
        v-for="(item, index) in edit.contact"
        v-bind:key="index"
      >
        <v-col cols="12">
      ...
      <v-row dense class="mt-2">
        <v-col cols="12" class="d-flex flex-column align-start">
          ...
          <textarea
            name="address"
            class="main-text-field w-100"
            rows="5"
            v-model="model.fields.appointment_address"
          ></textarea>
      ...
      <v-row v-if="relatedAppointments.length > 0">
        <span class="caption text-uppercase ml-3"> Related Appointments </span>
        <v-col
          cols="12"
          v-for="(item, ix) in relatedAppointments"
          v-bind:key="ix"
        >
          <v-row>
            <v-col>
          ...
          <div class="d-flex overflow-x-auto ga-5 mh-200">
            <v-card
              ...
              outlined
              min-width="200"
              v-for="(item2, ix2) in item.appointments"
              v-bind:key="ix2"
                  ...
                  <v-col>
                    <span>
                      <AppointmentStatusComp
                        v-bind:IsCancel="item2.fields.is_cancelled"
  ...
  methods: {
    ...
    searchInContacts(event) {
      ...
      if (event.target.value) {
        ...
        th.tempContacts = th.tempContacts.filter(
          (s) => s.SearchString.includes(val) //Daha önce adı ve soyadı eklediğimiz değişkende aratma yapıyorum.
        );
      }
    },
    ...
    validate() {
      ...
      if (th.edit) {
        var status = th.$createStatus(
          th.edit.appointment_date,
          th.edit.isCancel
        );
        if (
          status == th.$status.ISCOMING &&
          th.selectedStatus == th.$status.COMPLETED
        ) {
          th.errors.status =
            "Henüz Randevu Tarihi Geçmemiş, Tamamlanmış Yapamazsınız!";
        } else if (
          !moment
            .utc(th.edit.appointment_date)
            .isAfter(moment.utc(new Date())) &&
          th.selectedStatus == th.$status.ISCOMING
        ) {
          th.errors.status =
            "Randevu Tarihi Geçmiştir, Beklemede Yapamazsınız!";
        }
      }
      if (Object.keys(th.errors).length == 0) {
        //Eğer hata var ise yukarıda errors sınıflarının içerisine field olarak eklenmiş oluyor. Eğer eklenmediyse hata yok demektir.
        th.isValid = true;
      }
    },
    //#endregion
    ...
    async appointmentAction() {
      ...
      th.loading = true;
      try {
        var response = await th.sendRequest();
        ...
        Swal.fire({
          ...
          html:
            ...
            "Bir Hata Oluştu! Lütfen sistem yöneticinizle görüşünüz..",
        });
      }
      th.loading = false;
    },
    ...
    async sendRequest() {
      ...
      //UPDATE
        ...
        base.is_cancelled =
          th.selectedStatus == th.$status.CANCEL ? true : false;
        mdl.records = [
          {
            id: th.model.id,
            fields: base,
